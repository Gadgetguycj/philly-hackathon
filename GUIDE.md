...
- **Claude Code.** Needs a paid Claude plan; if you do not have one, pick Cursor. In a terminal opened in `hackathon` run the installer: on macOS `curl -fsSL https://claude.ai/install.sh | bash`, on Windows in PowerShell `irm https://claude.ai/install.ps1 | iex` (your prompt starts with `PS` in PowerShell). Open a new terminal in `hackathon`, run `claude`, and sign in when it asks.
...
- **Agent-operated infrastructure, 15.** Your GalaxyGate workflow history shows an `instance.provision` (5), an `app.provision` (5), and either an `app.power` row or more `app.deploy` rows than `app.provision` rows (5), because each provision brings exactly one `app.deploy` of its own, every later redeploy adds an `app.deploy`, and every restart adds an `app.power`. All rows must fall inside the build period. The panel shows them on your workspace's Workflows page; screenshot it. Your agent can also print them with `list_workspace_workflows`, and if a row has scrolled off, with `panel_request` GET `/v1/workspaces/<id>/workflows` and params `{"name": "instance.provision"}`.
