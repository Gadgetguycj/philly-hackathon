...
- **Agent-operated infrastructure, 15.** Your GalaxyGate workflow history shows an `instance.provision` (5), an `app.provision` (5), and either an `app.power` row or more `app.deploy` rows than `app.provision` rows (5), because each provision brings exactly one `app.deploy` of its own and every later redeploy or restart adds another. All rows must fall inside the build period. The panel shows them on your workspace's Workflows page; screenshot it. Your agent can also print them with `list_workspace_workflows`, and if a row has scrolled off, with `panel_request` GET `/v1/workspaces/<id>/workflows` and params `{"name": "instance.provision"}`.
