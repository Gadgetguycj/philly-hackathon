...
2. Copy the folder, using paths relative to the project folder (for example `-C philly-hackathon/demos roast`): `tar czf - -C <parent of folder> <folder name> | ssh -T -o BatchMode=yes -o StrictHostKeyChecking=accept-new -i .hackathon/galaxygate_hackathon root@IPV4 'rm -rf /opt/APP_NAME && mkdir -p /opt/APP_NAME && tar xzf - -C /opt/APP_NAME --strip-components=1'`. Before running it, confirm `APP_NAME` is one of the app names from `list_instance_apps` and contains only lowercase letters, digits and hyphens.
