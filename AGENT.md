...
Shell rule. Commands below are written for bash. On Windows your tool's shell is PowerShell, so for every bash command in this document write the command text to `.hackathon/cmd.sh` with `Set-Content -Encoding ascii` (never `Out-File` or `>`, which truncate long lines) and run it with `& 'C:\Program Files\Git\bin\bash.exe' .hackathon/cmd.sh`. That file exists once Git for Windows is installed; if it is missing, stop and tell the user to install Git for Windows from https://git-scm.com/downloads/win and start again. Pauses between polls are `Start-Sleep -Seconds N` in PowerShell.
