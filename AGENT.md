...
Shell rule. Commands below are written for bash. Run `uname -s` once. If it prints a name such as Linux, Darwin, MINGW64_NT or MSYS_NT, you are in bash: run every command directly and ignore the rest of this rule. If it errors, you are in PowerShell. Run every command in this document with the same tool that answered `uname -s`. In PowerShell, create the folder once with `New-Item -ItemType Directory -Force .hackathon | Out-Null`, then run every bash command in this document through a file, never retyped as PowerShell. Repeat all four lines every time, because your shell does not remember variables between commands, and keep the closing `'@` at the left margin:

```powershell
$bash = Join-Path (Split-Path (Split-Path (Get-Command git).Source)) 'bin\bash.exe'
$cmd = @'
<the bash command, pasted verbatim>
'@
[IO.File]::WriteAllText("$PWD\.hackathon\cmd.sh", $cmd.Replace("`r`n","`n") + "`n")
& $bash .hackathon/cmd.sh
```

If `Get-Command git` fails or the `$bash` path does not exist, stop and tell the user to install Git for Windows from https://git-scm.com/downloads/win with its default options and start again. Pauses between polls are `Start-Sleep -Seconds N` in PowerShell.
...
The panel always pulls an app's image from a registry when it deploys, so the new image goes into a registry that runs on the server itself and listens only on its loopback address. On Windows run this step's commands through bash.exe as the Shell rule says; the pipe in 12.2 does not survive PowerShell.

0. Recover the values. Read `HACKATHON.md` in the project for `INSTANCE_ID`, `IPV4`, `APP_NAME`, `APP_ID` and `APP_DOMAIN`. If it is missing, call `list_instances` with `q` set to the instance name, then `list_instance_apps`, and confirm the app's domain with the user before touching anything. Run every command in this step from the project folder, where `.hackathon` lives, not from inside the cloned repository. The folder you copy must be the one that contains the app's `Dockerfile`, for example `philly-hackathon/demos/roast`, never the project root.
1. Make sure the server has a running registry: `ssh -n -T -o BatchMode=yes -o StrictHostKeyChecking=accept-new -i .hackathon/galaxygate_hackathon root@IPV4 'docker start registry 2>/dev/null || docker run -d --restart unless-stopped -p 127.0.0.1:5000:5000 --name registry registry:2'`. If this command fails, show the user its output and stop.
2. Copy the folder, using paths relative to the project folder (for example `-C philly-hackathon/demos roast`): `tar czf - -C <parent of folder> <folder name> | ssh -T -o BatchMode=yes -o StrictHostKeyChecking=accept-new -i .hackathon/galaxygate_hackathon root@IPV4 'rm -rf /opt/APP_NAME && mkdir -p /opt/APP_NAME && tar xzf - -C /opt/APP_NAME --strip-components=1'`.
3. Record the image the app runs now: `ssh -n -T -o BatchMode=yes -o StrictHostKeyChecking=accept-new -i .hackathon/galaxygate_hackathon root@IPV4 'docker inspect -f "{{.Config.Image}}" APP_NAME'`; keep it as `OLD_IMAGE`. If it errors with no such object, run `docker ps --format '{{.Names}} {{.Image}}'` over ssh; if exactly one row is this app, use that container name for the rest of this step, otherwise show the list to the user and stop. Set `TAG` from `date -u +%Y%m%d%H%M%S`. Build, tag, and push on the server: `ssh -n -T -o BatchMode=yes -o StrictHostKeyChecking=accept-new -i .hackathon/galaxygate_hackathon root@IPV4 'docker build -t 127.0.0.1:5000/APP_NAME:TAG /opt/APP_NAME && docker push 127.0.0.1:5000/APP_NAME:TAG'`. If it fails, show the user the last lines and stop.
