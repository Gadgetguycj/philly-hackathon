...
Shell rule. Commands below are written for bash. Run `uname -s` once. If it prints a name such as Linux, Darwin or MINGW64_NT, you are in bash: run every command directly and ignore the rest of this rule. If it errors, you are in PowerShell. Then find Git Bash once with `$bash = Join-Path (Split-Path (Split-Path (Get-Command git).Source)) 'bin\bash.exe'`; if `Get-Command git` fails, stop and tell the user to install Git for Windows from https://git-scm.com/downloads/win with its default options and start again. Create the folder once with `New-Item -ItemType Directory -Force .hackathon | Out-Null`. Then run every bash command in this document through a file, never retyped as PowerShell:
...
Pauses between polls are `Start-Sleep -Seconds N` in PowerShell.
...
3. If the app was locked, or `delete_app` returned an error whose text contains `409`, leave that app alone. Set `APP_NAME=roast-2`, use `host_port` 8001 with `container_port` 8000, use `host_path` `/data/roast-2`, set `SUBDOMAIN` to the current value with `-2` appended, check that exact name with the `panel_request` call from step 7 without resetting `SUBDOMAIN`, and call `create_app` with those values. Record the new `APP_ID` and `APP_WORKFLOW`, replacing the old values.
...
2. Copy the folder, using paths relative to the project folder (for example `-C philly-hackathon/demos roast`): `tar czf - --exclude=.hackathon -C <parent of folder> <folder name> | ssh -T -o BatchMode=yes -o StrictHostKeyChecking=accept-new -i .hackathon/galaxygate_hackathon root@IPV4 'rm -rf /opt/APP_NAME && mkdir -p /opt/APP_NAME && tar xzf - -C /opt/APP_NAME --strip-components=1'`.
